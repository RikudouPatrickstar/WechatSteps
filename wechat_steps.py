...
if __name__ == "__main__":
    ...
    try:
        ...
        print('配置文件：读取出错，请检查语法')
        exit(1)

    # 设置变量
    user = config['login']['account']
    password = config['login']['password']
    if chinese_calendar.is_holiday(date=datetime.date.today()):
        # 节假日的步数范围
        min_steps = config['steps']['holiday']['min']
        max_steps = config['steps']['holiday']['max']
    else:
        # 工作日的步数范围
        min_steps = config['steps']['workday']['min']
        max_steps = config['steps']['workday']['max']
